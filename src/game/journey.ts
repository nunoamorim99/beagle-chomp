...
// equipped in the shop (game.ts's startJourneyLevel applies it, and restores
 ...
 * ratio; in practice every JOURNEY_LEVELS entry below sets
 ...
 */
export interface JourneyModifiers {
  speedMult: number;
 ...
 */
export const CLASSIC_MODIFIERS: JourneyModifiers = {
  speedMult: 1,
  ghostSpeedMult: 1,
  ghostCount: 3,
...
export type JourneyKind = "tour" | "twist";
...
export const JOURNEY_LEVELS: readonly JourneyLevel[] = [
  ...
  // JourneyModifiers' own doc comment) so a faster level is uniformly faster
 ...
 *  own doc comment: `challengeProgress === JOURNEY_LEVEL_COUNT` means "every
...
/**
 * Looks up a challenge level by index, clamped into [0, JOURNEY_LEVEL_COUNT
 ...
 */
export function getJourneyLevel(idx: number): JourneyLevel {
  const safe = Number.isFinite(idx) ? Math.floor(idx) : 0;
  const clamped = Math.max(0, Math.min(safe, JOURNEY_LEVEL_COUNT - 1));
  return JOURNEY_LEVELS[clamped];
}
...
export const JOURNEY_CHAPTERS: readonly JourneyChapter[] = buildChapters();

function buildChapters(): JourneyChapter[] {
  const PER_STAGE = 5;
  const out: JourneyChapter[] = [];
...
/** Which chapter a level index belongs to. Clamps like getJourneyLevel does
 *  rather than returning undefined, for the same reason: every caller here is
 *  rendering UI from a possibly-stale index. */
export function chapterForLevel(idx: number): JourneyChapter {
  const safe = Number.isFinite(idx) ? Math.floor(idx) : 0;
  const clamped = Math.max(0, Math.min(safe, JOURNEY_LEVEL_COUNT - 1));
 ...
 * JourneyLevel. Pure data, no import of mazes.ts itself (this module already
