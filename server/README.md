...
**Shipped and live**, and it owns rather more than a leaderboard now:

| Area | What it does |
|---|---|
| **Auth** | argon2id + opaque bearer tokens, single-use recovery codes, account deletion. |
| **Sessions** | A run is opened before it is played and finished against that row, so what a run *is* — its mode, its Journey level, the coat it was played in — is decided when it begins, not claimed at the end. |
| **Score validation** | `validation/plausibility.ts` reprices every submission against what the game can actually produce, and banks the coin award itself. The client's wallet is optimistic and reconciled. |
| **Challenges** | 114 goals whose progress is *derived* from `run_stats` by one conditional-aggregate query — no counters, no second copy of the truth. Claims are a table with a `(user_id, challenge_id)` primary key, which is what makes a double payout impossible rather than unlikely. |
| **Leaderboard** | One board: every player once, ranked on their best classic run. |
| **Push** | Web push for rank alerts, board nudges and release notes. |
| **Admin + metrics** | The portal's API, plus p95-per-route timings (see Observability below). |

**The game's constants are generated, not duplicated.** `catalog.generated.ts` is written
by `npm run sync` from the real `src/game` modules; `npm run test:catalog` fails on drift.
Changing `config.ts`, `mazes.json`, `journey.ts` or `challenges.ts` and skipping the sync
means honest runs start being rejected in production while everything passes locally.
...
src/
  ...
  catalog.generated.ts  the game's constants, GENERATED by `npm run sync`
  http/             cors.ts · errors.ts (the ApiError envelope) · body.ts
                    auth-middleware.ts · admin-middleware.ts · rate-limit.ts
                    metrics.ts — pure, framework-free request timings
                    metrics-middleware.ts — the Hono half of the above
  routes/           health · metrics · auth · profile · sessions
                    challenges · announcements · push · admin
  repo/             the ONLY place SQL lives (STACK.md §2.7)
  services/         business logic between routes and repos
  auth/             hash.ts · tokens.ts · recoveryCode.ts
  validation/       plausibility.ts — pure, heavily tested
                    wire.ts — reading a submission off the wire; pure ON PURPOSE,
                    so a DB-free test can reach it. A field not named here is
                    silently dropped, which once cost a whole release.
  notifications/    rankAlert.ts — who gets told, and who does not
  analytics/        aggregate.ts — the admin portal's numbers
migrations/         NNN_description.sql, applied in filename order
scripts/            migrate.ts · sync-game-constants.ts · the test suites
...
```bash
npm test                  # the pure suites — no database needed
npm run test:db           # auth against a real Postgres
npm run test:challenges:db
npm run test:sessions
npm run test:push
```

`npm run test:catalog` is the drift guard: it fails when `catalog.generated.ts` no longer
matches the game's own source, which is the failure that would otherwise only appear in
production as honest runs being rejected.
