// Scratch: every review view of the crab in ONE browser session (IDEA-054 v2).
//
...
//    crab's whole identity is stacked along the ONE axis a 59-degree camera
//    foreshortens.
...
//  - FIVE HUES. The crab's bread IS bodyMat, so the largest mass on the model
...
const OUT = process.env.OUT ?? ".img2threejs/crab/evidence";
...
await push("play", `model=crab&az=0&el=59&color=${ROSE}`, "play camera, el 59");
await push("play34", `model=crab&az=32&el=59&color=${ROSE}`, "play camera, az 32");
await push("fright", `model=crab&az=0&el=59&state=frightened&color=${ROSE}`, "frightened");
await push("clay", `model=crab&az=20&el=40&flat=1&color=${ROSE}`, "clay (form only)");
for (const [n, c] of HUES) {
  await push(`hue-${n}`, `model=crab&az=0&el=59&color=${c}`, `team ${n}`);
}
